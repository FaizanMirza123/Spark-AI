...
const containerVariants = {
  ...
  visible: {
    opacity: 1,
    scale: 1,
    transition: { staggerChildren: 0.08, delayChildren: 0.2, duration: 0.5, ease: [0.23, 1, 0.32, 1] as [number, number, number, number] },
  },
} as const
...
export const Card3D = React.forwardRef<HTMLDivElement, Card3DProps>(
  (
    { title, description, image, icon, theme = "primary", gradient, onClick, className, size = "md", variant = "default", disabled = false, loading = false, ...props },
    ref
  ) => {
    ...
    return (
      ...
      >
        ...
        >
          {image && (
            <img src={image} alt={title} className="h-full w-full object-cover transition-transform duration-500" loading="lazy" />
          )}
        </motion.div>
            ...
            <motion.div className="relative ml-auto" animate={{ scale: hovered ? 1.2 : 1 }} transition={{ duration: 0.3 }}>
              ...
              {!disabled && (
                <motion.div
                  ...
                  animate={{ scale: hovered ? [1, 1.4, 1] : 1, opacity: hovered ? [0.7, 0.3, 0.7] : 0.7 }}
        ...
        <motion.div
          ...
          style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.15) 0%, transparent 30%, transparent 70%, rgba(255,255,255,0.1) 100%)", transform: "translateZ(25px)" }}
        ...
        {!disabled && (
          <motion.div
            ...
            style={{ background: `linear-gradient(135deg, ${finalGradient})`, filter: "blur(15px)", transform: "translateZ(-5px)" }}
